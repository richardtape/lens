...
|-------------|--------|---------|
| iOS Simulator | LensIOS | ✓ |
| Physical iPhone | LensIOS | ✓ |
| macOS (My Mac) | LensMac | ✓ |
| Unit tests | LensCoreTests | ✓ 20 tests |

**What was added:**

| Layer | What exists |
|-------|-------------|
| `LensCore/Models/` | `Feed`, `FeedItem`, `Category`, `OfflineAsset`, `UserReadingPreferences`, `UserInterfacePreferences` — all `@Model` classes. Supporting enums: `SavedState`, `AppearanceOverride`, `LinkBehavior`, `FeedSortOrder`, `RetentionPolicy`. |
| `LensCore/Persistence/` | `PersistenceController` (App Group–backed `ModelContainer` + in-memory DEBUG factory), `CategorySeeder` (idempotent seeding of 10 predefined categories), `PreferenceStore` (singleton-row accessors for both preference records). |
| `LensIOS/LensApp.swift` | Creates `ModelContainer` on launch, injects via `.modelContainer()`, seeds categories. |
| `LensMac/LensMacApp.swift` | Same as LensApp. |
...
**App Group** `group.com.richardtape.lens` capability enabled in LensIOS and LensMac targets (entitlements files created by Xcode).
