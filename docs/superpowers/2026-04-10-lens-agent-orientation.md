...
**Phase 2 complete. Phase 3 in progress (Tasks 1–8 of 17 done).**

**Phase 3 WIP — Tasks completed:**

| Task | File(s) | Status |
|------|---------|--------|
| 1 | `LensCore/Models/UserReadingPreferences.swift` — `accentColorHex` field + test | ✓ |
| 2 | `LensUITests/` directory + placeholder (human must create Xcode target) | ✓ disk only |
...
**Next agent starts at Task 9** (plan: `docs/superpowers/plans/2026-04-12-lens-phase-3-timeline-ui.md`).

**Outstanding human action:** Task 2 requires the human driver to create the `LensUITests` Xcode target (File → New → Target → Unit Testing Bundle, "LensUITests", tested target: LensUI) before any LensUITests tests will run.

**Known issue flagged for Task 11:** `TimelineView.swift` must include an `.onChange(of: selectedItem)` block inside `ScrollViewReader` to emit `.itemDisplayed(itemId: item.id)` — see Task 8 commit message for the exact code snippet.
...
**Next:** Phase 3 Tasks 9–17 (see WIP section above).
...
│   ├── Shell/              # StubDestinationView.swift (RootTabView, RootSplitView — Tasks 9–10)
│   └── Timeline/           # ReaderStubView, TimelineFilter, TimelineState, ArticleRowView, …
